...
def get_policy(args):
    ...
    if args.nn == "dit": # DBC with DiT backbone
        # Note: Dimensions should match the pretrained model
        ...
        if args.conditioning == "concat":
            ...
            nn_condition = MultiImageObsCondition(shape_meta=args.shape_meta, emb_dim=embedding_dim,
                                                  rgb_model_name=args.rgb_model, rgb_weights=args.rgb_weights,
                                                  resize_shape=args.resize_shape, crop_shape=args.crop_shape,
                                                  random_crop=args.random_crop, use_group_norm=args.use_group_norm,
                                                  use_seq=args.use_seq).to(args.device)
            print("No conditioning specified! Falling back on the default MultiImageObsCondition!")
        ...
        try:
            ...
            for i, obs in enumerate(obs_list):
                ...
                if "agentview_rgb" in obs and self.args.shape_meta.obs.get("agentview", False):
                    obs_ = np.array(obs["agentview_rgb"]).astype(np.uint8)
                    obs_ = crop_resize(obs_, is_depth=False, output_size=(224, 224))
                    agentview_obs.append(obs_)

                # process agentview_depth
                if "agentview_depth" in obs and self.args.shape_meta.obs.get("depth", False):
                    obs_ = np.array(obs["agentview_depth"]).astype(np.float32)
                    obs_ = crop_resize(obs_, is_depth=True, output_size=(224, 224))
                    depth_obs.append(obs_)

                # process agentview_tactile
                if "tactile_rgb" in obs and self.args.shape_meta.obs.get("tactile", False):
                    print(f"Tactile shape: {np.array(obs['tactile_rgb']).shape}")
                    obs_ = np.array(obs["tactile_rgb"]).astype(np.float32)
                    tactile_obs.append(obs_)

                # process eef_pos
                if "eef_pos" in obs and self.args.shape_meta.obs.get("ee_pos", False):
                    obs_ = np.array(obs["eef_pos"]).astype(np.float32)
                    eef_pos_obs.append(obs_)

                # process eef_euler
                if "eef_euler" in obs and self.args.shape_meta.obs.get("ee_euler", False):
                    obs_ = np.array(obs["eef_euler"]).astype(np.float32)
                    ee_euler_obs.append(obs_)
            ...
            if tactile_obs:
                tactile_stacked = np.stack(tactile_obs, axis=0)  # (obs_steps, H, W, C)
                print(f"[tactile] Stacked shape: {tactile_stacked.shape}")

                nobs = self.dataset.normalizer['obs']['tactile'].normalize(tactile_stacked)
                nobs = nobs.transpose(0, 3, 1, 2)  # (obs_steps, C, H, W)
                nobs = np.expand_dims(nobs, 0)  # (1, obs_steps, C, H, W)
                obs_dict['tactile'] = torch.tensor(nobs, device=self.args.device, dtype=torch.float32)
                print(f"[tactile] Final tensor shape: {obs_dict['tactile'].shape}")
            ...
            if ee_euler_obs:
                ee_euler_stacked = np.stack(ee_euler_obs, axis=0)  # (obs_steps, 3)
                print(f"[ee_euler] Stacked shape: {ee_euler_stacked.shape}")

                nobs = self.dataset.normalizer['obs']['ee_euler'].normalize(ee_euler_stacked)
                nobs = np.expand_dims(nobs, 0)  # (1, obs_steps, 3)
                obs_dict['ee_euler'] = torch.tensor(nobs, device=self.args.device, dtype=torch.float32)
                print(f"[ee_euler] Final tensor shape: {obs_dict['ee_euler'].shape}")
...
def deploy(cfg: DeployConfig) -> None:
    ...
    server = PolicyServer(cfg, loaded_args)
