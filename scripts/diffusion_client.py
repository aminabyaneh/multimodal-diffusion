...
class RobotActionExecutor:
    ...
    Attributes:
        ...
        expected_ee_pose (np.ndarray): Internal state tracking the expected end-effector
    ...
    def __init__(self, robot):
        ...
        self.expected_ee_pose = np.concatenate([position, euler])  # [x, y, z, roll, pitch, yaw]
    ...
    def send_action(self, action: np.ndarray):
        ...
        """
        dpos = action[:3]
        drotation = action[3:6]
        grasp = 0 # TODO: not implemented

        # update internal state like frankaenv.step()
        self.expected_ee_pose[:3] += dpos
        self.expected_ee_pose[3:] = add_euler(drotation, self.expected_ee_pose[3:])  # shape-safe

        # convert to quaternion and send
        next_pos = self.expected_ee_pose[:3]
        next_quat = euler2quat(self.expected_ee_pose[3:])
...
def get_observation(pipeline, robot, digit):
    """
    Capture the current observation from the RealSense camera and robot's end-effector pose.
    """
    obs = {}
    ...
    # digit frame
    tactile_frame = cv2.cvtColor(digit.get_frame(), cv2.COLOR_BGR2RGB)
    ...
    obs["eef_euler"] = quat2euler(robot.get_ee_pose()[1]).tolist()
...
def apply_action(actions, executor, num_actions, step_time=0.2):
    """
...
def main():
    ...
    _ = input("All is set! Hit enter to start!!")
    ...
    try:
        while True:
            ...
            current_obs = get_observation(pipeline, robot, digit)
